...
class RegisterForm(forms.ModelForm):
    ...
    mobile_captcha = forms.CharField(label="验证码", widget=widgets.TextInput(attrs={"class":"form-control","style":"width: 250px;padding: 10px", "placeholder":"验证码", "error_messages": {"invalid": "验证码错误"}}))
    class Meta:
        model = User
        fields = ['username', 'mobile', 'password','email']
        widgets = {
            'username': widgets.TextInput(attrs={"class": "form-control", "placeholder": "请输入用户名", "error_messages": {"required": "用户名不能为空"}}),
            ...
            'email': widgets.TextInput(attrs={"class": "form-control", "placeholder": "请输入邮箱地址"}),
        }
...
class LoginForm(forms.Form):
    ...
    captcha = forms.CharField(label="验证码", widget=widgets.TextInput(
        attrs={"class": "form-control","placeholder": "验证码", "onblur": "check_captcha()",
